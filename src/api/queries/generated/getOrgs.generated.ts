...
export const useGetOrgsQuery = <TData = GetOrgsQuery, TError = unknown>(
  ...
  options?: Omit<UseQueryOptions<GetOrgsQuery, TError, TData>, 'queryKey'> & {
    queryKey?: UseQueryOptions<GetOrgsQuery, TError, TData>['queryKey'];
  },
) => {
  return useQuery<GetOrgsQuery, TError, TData>({
    queryKey: variables === undefined ? ['GetOrgs'] : ['GetOrgs', variables],
    queryFn: axiosRequest<GetOrgsQuery, GetOrgsQueryVariables>(GetOrgsDocument).bind(
      null,
      variables,
    ),
    ...options,
