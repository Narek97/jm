...
export const drawBackground = ({ sectionWidth }: { sectionWidth: number }) => {
  ...
  new Array(BACKGROUND_LINES_COUNT).fill({}).forEach((_, index) => {
    const divider = new Konva.Line({
      id: 'divider_' + index,
      points: [0, startY, sectionWidth, startY],
      stroke: '#e8e8e8',
      strokeWidth: 1,
    });
    startY += 50;
...
export const drawDividers = ({ sectionWidth }: { sectionWidth: number }) => {
  const linesGroup = new Konva.Group({
    x: 0,
    y: 0,
    id: 'lines_group',
  ...
  let startY = 280;
  new Array(Math.floor(sectionWidth / 279) - 1).fill({}).forEach((_, index) => {
    const divider = new Konva.Line({
      id: 'divider_column_' + index,
      points: [startY, 0, startY, 300],
      stroke: '#dddddd',
      strokeWidth: 0.4,
    });
    startY += 280 - index * 0.01;
    linesGroup.add(divider);
  });
  return { linesGroup };
};
