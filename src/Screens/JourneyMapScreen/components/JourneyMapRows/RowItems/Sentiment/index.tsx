...
import {
  ...
  useUpdatePersonaStateMutation,
} from '@/api/mutations/generated/updatePersonaState.generated';
import { PersonaStateEnum } from '@/api/types.ts';
import {
  EMOJI_WIDTH,
  EMOJIS,
  EMOTION_TYPES,
  SENTIMENT_HEIGHT,
} from '@/Screens/JourneyMapScreen/components/JourneyMapRows/JourneyMapSentimentRow/constants.ts';
import {
  createConnector,
...
const Sentiment: FC<ISentiment> = ({
  ...
  const sectionWidth = columnsCount * width + columnsCount;
  ...
  const drawDividersOnSentiment = useCallback(() => {
    const { linesGroup } = drawDividers({
      sectionWidth,
    });
    layerRef.current?.add(linesGroup);
  }, [sectionWidth]);
  ...
  const onHandleUpdateJourneyMap = ({
    emojiId,
    parentId,
    state,
  }: {
