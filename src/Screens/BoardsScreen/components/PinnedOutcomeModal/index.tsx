...
const BoardPinnedOutcomesModal: FC<IBoardPinnedOutcomesModal> = ({
  ...
  return (
    <BaseWuModal
      ...
      <div data-testid={'pinned-outcomes'} className={'[height:26.875rem]'}>
        {isLoadingPinnedOutcomes ? (
          ...
          <div
            className={
              '[height:22.875rem] overflow-auto px-6 border-b border-[var(--base-extra-light-gray-2)]'
            }>
            <ul data-testid={'outcomes-section-list'}>
              {pinnedOutcomes?.getBoardOutcomesStat.outcomeStats?.map(outcomeGroupItem => (
                <li
                  ...
                  className={`flex items-center justify-between border border-[var(--base-light-gray-color-2)] rounded mb-4 !pt-[0.625rem]
                   !px-3 !pb-3 text-[var(--base-gray-color)] cursor-pointer hover:!border-[var(--primary)]`}>
                  <img
                    className={'w-4 h-auto'}
                    src={outcomeGroupItem?.icon}
                    alt={'outcome_image'}
                  />
                  <div className={'truncate max-w-[12.5rem]'}>{outcomeGroupItem?.name}</div>
