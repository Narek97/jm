...
import {
  InterviewFormType,
  InterviewType,
} from "@/Screens/InterviewsScreen/types.ts";
...
const CreateInterviewModal: FC<ICreateInterviewModal> = ({
  ...
  const [selectedSliderCardId, setSelectedSliderCardId] = useState<
    number | null
  ...
  const { data: dataAiModels, isLoading: isLoadingAiModels } =
  ...
  const {
    ...
    },
    {
      ...
      getNextPageParam: function (
        lastPage: GetMyBoardsQuery,
        allPages: GetMyBoardsQuery[],
      ) {
        return lastPage.getMyBoards.boards.length < BOARDS_LIMIT
          ? undefined
          : allPages.length;
      },
      initialPageParam: 0,
  ...
  return (
    ...
    >
      <CustomModalHeader
        title={<>{interview ? "View" : "Create"} interview</>}
      />
      ...
      <div className={"create-interview-modal"}>
        ...
        >
          ...
          <div className={"create-interview-modal--content-dropdown"}>
            ...
            <span className={"validation-error"}>
              {errors.aiJourneyModelId?.message}
            </span>
          </div>

          <div
            className={`create-interview-modal--content-input`}
            key={"transcript"}
          >
            <label
              className={"create-interview-modal--content-input--label"}
